...
export const DesignItem: React.FC<DesignItemProps> = ({
    ...
    <motion.div className="fixed inset-0 z-50 overflow-hidden dark:bg-black bg-white">
      <div className="h-full flex flex-col md:flex-row p-0 md:p-10 relative">
        ...
        <div className="w-full md:w-72 p-6 md:p-0 md:pr-6 md:mt-10 md:ml-12 flex flex-col justify-between">
          <div>
            <motion.h3 layoutId="title" className="text-lg dark:text-white text-black mb-3 font-medium">
              {title}
            </motion.h3>
            <motion.p layoutId="description" className="dark:text-neutral-400 text-neutral-600 mb-6 leading-relaxed">
              {description}
            </motion.p>
            <motion.div layoutId="tags" className="flex flex-wrap gap-2">
              {tags.map((tag, index) => (
                <span
                  key={index}
  ...
  const MinimizedView = () => (
    <div className="mb-12 mt-12" ref={componentRef}>
      <motion.h3 layoutId="title" className="text-lg mb-2 text-black dark:text-night-heading">
        {title}
      </motion.h3>
      <motion.p layoutId="description" className="mb-6 text-neutral-700 dark:text-neutral-400">
        {description}
      </motion.p>
      <motion.div layoutId="tags" className="flex flex-wrap sm:flex-row gap-2 mb-4">
        {tags.map((tag, index) => (
          <span
            key={index}
      ...
      <div className="relative group">
        <motion.div layoutId="container" className="bg-white rounded-xl items-center flex justify-center dark:bg-black border border-gray-200 dark:border-neutral-900 p-4 md:min-h-[600px] min-h-[400px]">
          {children}
        </motion.div>
        <motion.button
          layoutId="control-button"
